...
double mse(std::vector<double> const &a, std::vector<double> const &b)
{
    double s = 0;
...
double psnr_uint8(std::vector<double> const &original, std::vector<double> const &reconstructed)
{
    double const e = mse(original, reconstructed);
    if (e <= 0)
        return 999.0;
...
ResidualSpectrum analyze_residual_spectrum(std::vector<double> const &residual, int W, int H)
{
    // Pad to next pow-2 separately for each axis.
    int const padW = next_pow2(W);
    int const padH = next_pow2(H);
    ...
    for (int y = 0; y < padH; ++y) {
        radix2_fft(work_re.data() + y * padW, work_im.data() + y * padW, padW, kForward);
    }
    ...
    double const total = std::accumulate(mag.begin(), mag.end(), 0.0);

    // Sort descending and accumulate top-N percentages.
    std::sort(mag.begin(), mag.end(), std::greater<double>());
    int const n_1pct = std::max(1, static_cast<int>(mag.size()) / 100);
    int const n_5pct = std::max(1, static_cast<int>(mag.size()) / 20);
    double const e_1 = std::accumulate(mag.begin(), mag.begin() + n_1pct, 0.0);
    double const e_5 = std::accumulate(mag.begin(), mag.begin() + n_5pct, 0.0);

    return ResidualSpectrum{total, e_1, e_5, total > 0 ? e_1 / total : 0.0};
}

// Patch experiment: take residual, FFT, keep top-J bins, inverse FFT,
// add back to reconstruction. Returns PSNR after patching.
double psnr_after_residual_patch(std::vector<double> const &original, std::vector<double> const &reconstructed, int W,
                                 int H, int J)
{
    ...
    std::iota(idx.begin(), idx.end(), 0);
    std::partial_sort(idx.begin(), idx.begin() + J, idx.end(), [&](int a, int b) {
        double const ma = work_re[a] * work_re[a] + work_im[a] * work_im[a];
        double const mb = work_re[b] * work_re[b] + work_im[b] * work_im[b];
        return ma > mb;
    });
    ...
    for (int y = 0; y < padH; ++y) {
        radix2_fft(work_re.data() + y * padW, work_im.data() + y * padW, padW, kInverse);
    }
    for (int x = 0; x < padW; ++x) {
        for (int y = 0; y < padH; ++y) {
            col_re[y] = work_re[y * padW + x];
...
std::vector<double> input_disk(int W, int H)
{
    ...
    double const cx = W / 2.0, cy = H / 2.0;
    double const r = std::min(W, H) * 0.25;
    for (int y = 0; y < H; ++y)
        for (int x = 0; x < W; ++x) {
...
std::vector<double> input_pink_noise(int W, int H)
{
    ...
    std::vector<double> out(W * H);
    for (int i = 0; i < W * H; ++i)
        out[i] = static_cast<double>(tile[i]);
    return out;
}
...
std::vector<double> input_bilateral_edge(int W, int H)
{
    ...
    for (int y = 0; y < H; ++y)
        for (int x = 0; x < W; ++x) {
            int const j = next();
            src[y * W + x] = static_cast<std::uint8_t>((x < W / 2 ? 60 : 200) + j - 16);
        }
    ...
    std::vector<double> out(W * H);
    for (int i = 0; i < W * H; ++i)
        out[i] = static_cast<double>(dst[i]);
    return out;
}
...
void run_trial(char const *name, std::vector<double> const &original, int W, int H, int K, std::vector<Trial> &results)
{
    // Forward DCT-II.
    ...
    double const psnr_t = psnr_uint8(original, reconstructed);

    // Residual spectrum analysis.
    std::vector<double> residual(W * H);
    for (int i = 0; i < W * H; ++i)
        residual[i] = original[i] - reconstructed[i];
    ResidualSpectrum const spec = analyze_residual_spectrum(residual, W, H);

    // Patch experiment: keep top-J residual modes (J = K / 4).
    int const J = std::max(1, K / 4);
    double const psnr_p = psnr_after_residual_patch(original, reconstructed, W, H, J);

    results.push_back(Trial{name, W, H, W * H, K, static_cast<double>(K) / (W * H), psnr_t, psnr_p, psnr_p - psnr_t,
                            spec.concentration});
}

void run_all_trials(char const *name, std::vector<double> input, int W, int H, std::vector<Trial> &results)
{
    int const total = W * H;
    for (double r : {0.5, 0.25, 0.125, 0.0625, 0.03125, 0.015625}) {
        int const K = std::max(1, static_cast<int>(total * r));
        run_trial(name, input, W, H, K, results);
    }
}

void print_report_row(Trial const &t)
{
    std::fprintf(stderr, "| %-20s | %5dx%-5d | %8d | %8.2f%% | %7.2f | %7.2f | %+7.2f | %7.2f%% |\n", t.name, t.W, t.H,
                 t.K, t.ratio * 100.0, t.psnr_truncated, t.psnr_patched, t.patched_uplift,
