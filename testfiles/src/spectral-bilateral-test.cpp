...
int max_horiz_gradient(const std::uint8_t *m, int W, int H)
{
    int g = 0;
    for (int y = 0; y < H; ++y) {
        for (int x = 0; x + 1 < W; ++x) {
            const int d = std::abs(m[y * W + x + 1] - m[y * W + x]);
            g = std::max(g, d);
        }
    }
    return g;
...
TEST(SpectralBilateral, EdgePreservedAtSmallSigmaRange)
{
    ...
    const int src_step = max_horiz_gradient(src.data(), W, H);
    const int dst_step = max_horiz_gradient(dst.data(), W, H);
    std::fprintf(stderr,
        "[bilateral edge sigma_r=16] src-step=%d dst-step=%d\n",
        src_step, dst_step);
...
TEST(SpectralBilateral, LargeSigmaRangeApproachesPlainHeat)
{
    ...
    const int dst_step = max_horiz_gradient(dst.data(), W, H);
    std::fprintf(stderr,
        "[bilateral edge sigma_r=1e6] dst-step=%d (should be << 255)\n",
        dst_step);
    EXPECT_LT(dst_step, 100)
        << "Gaussian-limit (sigma_range=1e6) should smear the edge";
}
...
TEST(SpectralBilateral, RGBAFlatRegionInvariant)
{
    ...
    for (int i = 0; i < W * H; ++i) {
        src[i*4 + 0] = 100; src[i*4 + 1] = 150;
        src[i*4 + 2] = 200; src[i*4 + 3] = 255;
    }
...
TEST(SpectralBilateral, RGBAColorEdgePreserved)
{
    ...
    for (int y = 0; y < H; ++y) {
        for (int x = 0; x < W; ++x) {
            ...
            // BGRA: B G R A
            if (x < W / 2) { p[0] = 0;   p[1] = 0; p[2] = 255; p[3] = 255; }
            else           { p[0] = 255; p[1] = 0; p[2] = 0;   p[3] = 255; }
        }
    }
    bilateral_bgra(W, H, src.data(), W * 4, dst.data(), W * 4,
                    /*sigma_spatial=*/4.0, /*sigma_range=*/16.0);
    ...
    for (int y = 0; y < H; ++y) {
        for (int x = 0; x + 1 < W; ++x) {
            const int r0 = dst[(y * W + x)     * 4 + 2];
            const int r1 = dst[(y * W + x + 1) * 4 + 2];
            max_r_grad = std::max(max_r_grad, std::abs(r1 - r0));
        }
    }
    std::fprintf(stderr,
        "[bilateral colour-edge sigma_r=16] max R-gradient=%d\n",
        max_r_grad);
