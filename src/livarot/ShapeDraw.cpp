...
Shape::Plot (double ix, double iy, double ir, double mx, double my, bool doPoint,
	     bool edgesNo, bool pointsNo, bool doDir, char const *fileName)
{
  ...
  fprintf(outFile,"   inkscape:version=\"1.0\"\n");
  ...
  if ( pointsNo ) {
		for (int i=0;i<numberOfPoints();i++) {
			double   ph=(getPoint(i).x[0]-ix)*ir+mx;
			double   pv=(getPoint(i).x[1]-iy)*ir+my;
      fprintf(outFile,"     <text x=\"%f\" y=\"%f\" font-size=\"12\" fill=\"red\" >\n",ph-2,pv+1);
      fprintf(outFile,"%i\n",i);
  ...
  }
	{
        ...
        fprintf(outFile,"     <path d=\"M %f,%f %f,%f\" stroke=\"black\" stroke-width=\"0.5\" />\n",sh,sv,endh,endv);
			} else {
        fprintf(outFile,"     <path d=\"M %f,%f %f,%f\" stroke=\"black\" stroke-width=\"0.5\" />\n",sh,sv,eh,ev);
			}
		}
  ...
  if ( edgesNo ) {
		for (int i=0;i<numberOfEdges();i++) {
			...
			double   eh=(getPoint(enP).x[0]-ix)*ir+mx;
			double   ev=(getPoint(enP).x[1]-iy)*ir+my;
      fprintf(outFile,"     <text x=\"%f\" y=\"%f\" font-size=\"12\" fill=\"blue\" >\n",(sh+2*eh)/3+2,(sv+2*ev)/3);
      fprintf(outFile,"%i\n",i);
      fprintf(outFile,"     </text>\n");
		}
  }
