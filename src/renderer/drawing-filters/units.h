...
#ifndef SEEN_INKSCAPE_RENDERER_DRAWING_FILTERS_UNITS_H
#define SEEN_INKSCAPE_RENDERER_DRAWING_FILTERS_UNITS_H
...
public:
    Units();
    Units(SPFilterUnits const filterUnits, SPFilterUnits const primitiveUnits);
     ...
     */
    void set_item_bbox(Geom::OptRect const &bbox) { item_bbox = bbox; }

    /**
     * Sets the filter effects area in user coordinates
     */
    void set_filter_area(Geom::OptRect const &area) { filter_area = area; }

    /**
     * Sets the render area for the painting surface
     */
    void set_render_area(Geom::Rect const &area) { render_area = area; }
    ...
    /**
     * Get a transform from item to filter pixels, only if it's not a translation.
     */
    std::optional<Geom::Affine> get_matrix_item2filter() const;
    ...
    /**
     * A dubious calculation to get the amount of pixels needed to store
     * a reverse translation of the original graphic for filters.
     *
     * TODO: Investigate removing this
     */
    Geom::OptRect get_slot_box() const;

    /**
     * Return the render area as set.
     */
    Geom::OptRect get_render_area() const { return render_area; }
...
private:
    ...
    Geom::OptRect render_area;
};

} // namespace Inkscape::Renderer::Filters

#endif /* SEEN_INKSCAPE_RENDERER_DRAWING_FILTERS_UNITS_H */
/*
  Local Variables:
  mode:c++
