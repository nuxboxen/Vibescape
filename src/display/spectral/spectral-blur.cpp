...
void apply_heat_kernel_a8(int W, int H, std::uint8_t *buf, double sigma_x, double sigma_y)
{
    ...
    for (int y = 0; y < H; ++y) {
        ...
        double const *src_row = work.data() + y * padW;
        for (int x = 0; x < W; ++x) {
            double const v = std::round(src_row[x]);
            int const iv = (v < 0.0) ? 0 : (v > 255.0) ? 255 : static_cast<int>(v);
               ...
               double sigma_x, double sigma_y)
{
    ...
    if (sigma_x <= 0.0 && sigma_y <= 0.0) {
        for (int y = 0; y < H; ++y) {
            std::memcpy(dst + y * dst_stride, src + y * src_stride, static_cast<std::size_t>(W) * 4);
        }
        return;
    }

    std::size_t const plane = static_cast<std::size_t>(W) * static_cast<std::size_t>(H);
    ...
    // Reinterleave.
    for (int y = 0; y < H; ++y) {
        std::uint8_t const *b_row = b.data() + y * W;
        std::uint8_t const *g_row = g.data() + y * W;
        std::uint8_t const *r_row = r.data() + y * W;
        std::uint8_t const *a_row = a.data() + y * W;
