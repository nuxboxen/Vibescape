...
bool style_needs_group(SPStyle const *style)
{
    ...
    return (double)style->opacity < 1.0 || get_blendmode(style->mix_blend_mode.value) ||
...
Document::get_group_graphics_state(SPStyle const *style, std::optional<CapyPDF_TransparencyGroupId> soft_mask)
{
    ...
    if ((double)style->opacity < 1.0) {
        gstate.set_ca((double)style->opacity);
        gs_used = true;
    }
    if (gs_used) {
...
std::optional<CapyPDF_GraphicsStateId> Document::get_shape_graphics_state(SPStyle const *style)
{
    ...
    if (auto soft_mask = style_to_transparency_mask(style, nullptr)) {
        ...
        if ((double)style->fill_opacity < 1.0) {
            gstate.set_ca((double)style->fill_opacity);
            gs_used = true;
        }
        if ((double)style->stroke_opacity < 1.0) {
            gstate.set_CA((double)style->stroke_opacity);
            gs_used = true;
        }
    }
    if (gs_used) {
...
void DrawContext::set_paint_style(StyleMap const &map, SPStyle const *style, SPStyle const *context_style)
{
    ...
    if (map.contains(SPAttr::FILL)) {
        if (auto color = _doc.get_paint(style->fill, context_style, get_softmask((double)style->fill_opacity))) {
            _ctx.set_nonstroke(*color);
        }
    }
    if (map.contains(SPAttr::STROKE)) {
        if (auto color = _doc.get_paint(style->stroke, context_style, get_softmask((double)style->stroke_opacity))) {
